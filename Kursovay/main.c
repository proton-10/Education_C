		...
		if(str[0] == '-')
		{
			switch (str[1])
			{
				...
				case 'h':
				{
					...
					printf("Атрибуты:\n");
					printf("-h  - получение информации о возможных ключах запуска с кратким описанием их назначения;\n\n");
					printf("-f <file_name.csv>  - указание файла для обработки, где file_name – имя файла.\nЕсли в этом режиме не добавлена опция «-m» или «-y», то будет доступен полный функционал приложения;\n\n");
					printf("-m <xxxx/xx> - указание месяца и года для получения статистики по конкретному месяцу конкретного года,\nгде xxxx – год, xx - месяц;\n\n");
					printf("-y <xxxx>  - указание года для получения статистики по конкретного года, где xxxx – год;\n\n");
					return 0;
				}break;
